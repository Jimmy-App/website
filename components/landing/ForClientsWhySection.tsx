...
import {
  DEFAULT_FOR_CLIENTS_WHY_CONTENT,
  DEFAULT_FOR_CLIENTS_WHY_POINTS,
  type ForClientsWhyContent,
  type ForClientsWhyIconKey,
} from "./forClientsContent";

const whyIconMap: Record<ForClientsWhyIconKey, LucideIcon> = {
  zoomIn: ZoomIn,
  clock3: Clock3,
  wifiOff: WifiOff,
};

type ForClientsWhySectionProps = {
  content?: ForClientsWhyContent | null;
};

const ForClientsWhySection = ({ content }: ForClientsWhySectionProps) => {
  const badgeText =
    content?.badgeText ?? DEFAULT_FOR_CLIENTS_WHY_CONTENT.badgeText;
  const title = content?.title ?? DEFAULT_FOR_CLIENTS_WHY_CONTENT.title;
  const sourcePoints =
  ...
  const resolvedPoints = sourcePoints.map((point, index) => {
    const fallbackPoint =
      DEFAULT_FOR_CLIENTS_WHY_POINTS[index] || DEFAULT_FOR_CLIENTS_WHY_POINTS[0];
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-10 grid gap-5 md:mt-12 md:grid-cols-3">
          {resolvedPoints.map((point, index) => {
            const Icon = whyIconMap[point.iconKey] || ZoomIn;
